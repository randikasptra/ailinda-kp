...
<div class="mt-24 px-4 md:px-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div class="flex items-center">
            <div class="p-3 rounded-xl bg-gradient-to-r from-emerald-400 to-emerald-600 shadow-lg mr-4">
                <i class="fas fa-chart-line text-white text-2xl"></i>
            </div>
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Laporan Rekap Surat Izin Masuk</h1>
                <p class="text-gray-600 mt-1 text-sm md:text-base">Analisis dan monitoring data surat izin masuk siswa</p>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-3 border border-gray-200">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800"><?= count($izinMasuk ?? []) ?> Records</p>
                    </div>
                </div>
            </div>
            <a href="<?= base_url('admin/laporan/keluar') ?>" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-all">Lihat Izin Keluar</a>
        </div>
    </div>
           ...
           <!-- Tanggal Mulai -->
            <div>
                <label for="start_date" class="block text-sm font-medium text-gray-700 mb-2">
                    <i class="fas fa-calendar-day text-emerald-500 mr-1"></i> Dari Tanggal
                </label>
                <input type="date" name="start_date" id="start_date" 
                    value="<?= esc(service('request')->getGet('start_date') ?: date('Y-m-d')) ?>" 
            ...
            <div>
                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">
                    <i class="fas fa-calendar-day text-emerald-500 mr-1"></i> Sampai Tanggal
                </label>
                <input type="date" name="end_date" id="end_date" 
                    value="<?= esc(service('request')->getGet('end_date') ?: date('Y-m-d')) ?>" 
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
            </div>
            ...
            <div>
                <label for="search_nama" class="block text-sm font-medium text-gray-700 mb-2">
                    <i class="fas fa-search text-emerald-500 mr-1"></i> Cari Siswa
                </label>
            ...
            <!-- Action Buttons -->
            <div class="flex flex-col md:flex-row items-end gap-2 lg:col-span-4">
                <button type="submit" name="filter" value="1" 
                        class="flex-1 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-4 py-2 rounded-lg hover:from-emerald-700 hover:to-emerald-800 transition-all shadow-sm flex items-center justify-center gap-2">
                    <i class="fas fa-filter text-sm"></i>
                    Terapkan
                </button>
                <button type="submit" name="export" value="1" 
                        class="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-4 py-2 rounded-lg hover:from-emerald-600 hover:to-emerald-700 transition-all shadow-sm flex items-center justify-center gap-2">
    ...
    <!-- Stats Overview (Hanya untuk Masuk) -->
