...
<div class="ml-64 mt-16 px-8 py-6">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-[#1E5631] to-[#2E7D32]">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">Nama
                            Siswa</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">Kelas
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">Jam
                            Keluar</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">
                            Status</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">Poin
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-white/90 uppercase tracking-wider">
                            Dicatat</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-white/90 uppercase tracking-wider">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($historyList as $row): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        ...
                                        <span
                                            class="text-[#1E5631] font-medium"><?= strtoupper(substr(esc($row['nama']), 0, 1)) ?></span>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full"><?= esc($row['kelas']) ?></span>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <i data-lucide="clock"
                                    class="w-4 h-4 mr-1 <?= empty($row['waktu_kembali_siswa']) ? 'text-red-400' : 'text-green-500' ?>"></i>
                            ...
                            <td class="px-6 py-4">
                                <?php if (empty($row['waktu_kembali_siswa'])): ?>
                                    <span class="px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">Belum
                                        Kembali</span>
                                <?php elseif (strtotime($row['waktu_kembali_siswa']) > strtotime($row['waktu_kembali'])): ?>
                                    <span
                                        class="px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">Terlambat</span>
                                <?php else: ?>
                                    <span class="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">Tepat
                                        Waktu</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-1 text-xs font-medium <?= ($row['poin_pelanggaran'] ?? 0) > 0 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' ?> rounded-full">
    ...
    <div id="editModal" class="fixed inset-0 hidden z-50 bg-black/50 flex items-center justify-center">
        <form method="post" id="editForm" class="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl">
            ...
            <div class="mb-4">
                <label class="text-sm font-medium">Poin Pelanggaran</label>
                <input type="number" name="poin_pelanggaran" id="edit_poin" class="w-full border px-3 py-2 rounded">
            </div>
