...
{
    $this->autoDeleteOldData();

    $page_keluar = $this->request->getGet('page_keluar') ?? 1;
    $page_masuk  = $this->request->getGet('page_masuk') ?? 1;

    // --- Surat Izin Keluar ---
    $builderKeluar = $this->suratIzinModel->getRekapanKeluarToday();

    $suratIzin = $builderKeluar->paginate(5, 'keluar', $page_keluar);
    $pager_keluar = $this->suratIzinModel->pager;
    $total_izin_keluar = $this->suratIzinModel->pager->getTotal('keluar'); // bisa juga countAllResults()
    ...
    // --- Surat Izin Masuk ---
    $builderMasuk = $this->suratIzinMasukModel
        ->where('DATE(surat_izin_masuk.created_at)', date('Y-m-d'));

    $suratIzinMasuk = $builderMasuk->paginate(5, 'masuk', $page_masuk);
    ...
    $suratIzinKemarin = $this->suratIzinModel
        ...
        ->where('DATE(surat_izin.created_at)', $kemarin)
        ->findAll();
    ...
    $suratIzinMasukKemarin = $this->suratIzinMasukModel
        ...
        ->where('DATE(surat_izin_masuk.created_at)', $kemarin)
        ->findAll();
